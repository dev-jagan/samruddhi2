...
export function CalendarGrid({
    ...
    const getDateStatus = (dayNum: number) => {
        ...
        return matchedRecord ? matchedRecord.status : "available";
    };

    const handleNextMonth = () => setCurrentCalDate(new Date(year, monthIdx + 1, 1));
    const handlePrevMonth = () => setCurrentCalDate(new Date(year, monthIdx - 1, 1));
    ...
    const handleCellClick = (dayNum: number) => {
        ...
        if (status === "booked" || status === "blocked") {
            showToast(`This date is ${status === 'booked' ? 'fully booked' : 'blocked by the owner'}. Select another date.`, "error");
            return;
        }
        ...
        if (!checkInDate || (checkInDate && checkOutDate)) {
            ...
            setCheckOutDate("");
            showToast(`Check-in: ${dateStr}. Now select check-out.`, "info");
        } else {
            if (new Date(dateStr) <= new Date(checkInDate)) {
                ...
                if (blockFound) {
                    showToast("A booked/blocked date occurs within this range. Select continuous available dates.", "error");
                } else {
                    setCheckOutDate(dateStr);
                    showToast("Dates selected! Proceed with booking.", "success");
                }
            }
        }
    };
    ...
    return (
        <div className={`rounded-2xl p-2 ${dm ? 'bg-stone-800 border border-stone-700' : 'bg-stone-50/50 border border-stone-100'}`}>

            <div className="flex justify-between items-center mb-3 px-1">
                <button onClick={handlePrevMonth} className={`p-1 rounded transition font-bold ${dm ? 'hover:bg-stone-700 text-stone-300' : 'hover:bg-stone-200 text-stone-600'}`}>‹</button>
                <span className={`font-serif text-sm font-semibold ${dm ? 'text-stone-100' : 'text-stone-900'}`}>{monthName} {year}</span>
                <button onClick={handleNextMonth} className={`p-1 rounded transition font-bold ${dm ? 'hover:bg-stone-700 text-stone-300' : 'hover:bg-stone-200 text-stone-600'}`}>›</button>
            </div>

            <div className="grid grid-cols-7 gap-1 text-center font-mono text-[9px] font-bold mb-1">
                {weekDayLabels.map((lbl, i) => (
                    <span key={i} className={`py-1 ${dm ? 'text-stone-400' : 'text-stone-400'}`}>{lbl}</span>
                ))}
            </div>
            ...
            <div className="grid grid-cols-7 gap-1">
                {dayCells.map((cell, idx) => {
                    ...
                    if (cell.status === "booked") {
                        ...
                        bgClass = `${dm ? 'bg-stone-700 border-stone-600 text-stone-400' : 'bg-stone-200 border-stone-300 text-stone-600'} cursor-not-allowed`;
                    }

                    if (isSelectedCheckin || isSelectedCheckout) {
                        bgClass = "bg-stone-950 border-stone-950 text-white font-bold ring-2 ring-stone-400 ring-offset-1";
...
export function getAmenityIcon(iconName: string, darkMode?: boolean) {
    const cls = darkMode ? "w-5 h-5 text-stone-300" : "w-5 h-5 text-stone-700";
    switch (iconName) {
        case "snowflake":
            return (
